...
import (
	...
	omni "github.com/voxgig-sdk/eu-vat-validation-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.EuVatValidationError); is {
				out := map[string]any{
					"name":    "EuVatValidationError",
...
func omniCtx(arg any, client *sdk.EuVatValidationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.EuVatValidationSDK); is {
			client = live
