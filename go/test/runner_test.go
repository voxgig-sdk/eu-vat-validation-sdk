...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("EU_VAT_VALIDATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("EU_VAT_VALIDATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("EU_VAT_VALIDATION_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.EuVatValidationSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.EuVatValidationError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.EuVatValidationSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.EuVatValidationError{Msg: msg, Code: code}
}
