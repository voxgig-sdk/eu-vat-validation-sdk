...
local VatEntity = {}
VatEntity.__index = VatEntity


function VatEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, VatEntity)
  self._name = "vat"
  self._client = client
...
function VatEntity:get_name()
  return self._name
end


function VatEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return VatEntity.new(self._client, opts)
end
...
function VatEntity:mark_deleted()
  self._deleted = true
end


function VatEntity:deleted()
  return true == self._deleted
end


function VatEntity:data_set(args)
  if args ~= nil then
...
function VatEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function VatEntity:match_set(args)
  if args ~= nil then
...
function VatEntity:stream(action, args, callopts)
...
---@param reqmatch VatLoadMatch
---@param ctrl? table
---@return Vat
---@return string? err
function VatEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function VatEntity:_run_op(ctx, post_done)
  local utility = self._utility
