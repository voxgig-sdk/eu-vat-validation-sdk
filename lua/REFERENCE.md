# EuVatValidation Lua SDK Reference

Complete API reference for the EuVatValidation Lua SDK.
...
```lua
local validate_format = client:ValidateFormat(nil)
```
...
```lua
local result, err = client:ValidateFormat():load({ country = "country", number = "number" })
```
...
```lua
local result, err = client:Vat():load({ country = "country", number = "number" })
```
