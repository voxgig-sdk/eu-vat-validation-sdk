...
describe("ValidateFormatEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:ValidateFormat(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = validate_format_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "validate_format." .. _op, _live and "live" or "unit")
    ...
    local validate_format_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.validate_format")))
    local validate_format_ref01_data = nil
    if #validate_format_ref01_data_raw > 0 then
      validate_format_ref01_data = helpers.to_map(validate_format_ref01_data_raw[1][2])
    end
...
function validate_format_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/validate_format/ValidateFormatTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read validate_format test data: " .. entity_data_file)
  end
