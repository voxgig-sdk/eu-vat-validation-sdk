...
// EuVatValidation SDK retry feature
...
class EuVatValidationRetryFeature extends EuVatValidationBaseFeature
{
    ...
    public function init(EuVatValidationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (EuVatValidationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(EuVatValidationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(EuVatValidationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
