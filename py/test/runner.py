# EuVatValidation SDK test runner
...
class EuVatValidationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            EuVatValidationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = EuVatValidationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = EuVatValidationTestRunner.getenv("EU_VAT_VALIDATION_TEST_LIVE")
        override = EuVatValidationTestRunner.getenv("EU_VAT_VALIDATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = EuVatValidationTestRunner.getenv(key)
        ...
        explain = EuVatValidationTestRunner.getenv("EU_VAT_VALIDATION_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["EU_VAT_VALIDATION_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if EuVatValidationTestRunner._test_control is not None:
            return EuVatValidationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                EuVatValidationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            EuVatValidationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = EuVatValidationTestRunner.load_test_control()
...
def load_env_local():
    EuVatValidationTestRunner.load_env_local()


def env_override(m):
    return EuVatValidationTestRunner.env_override(m)


def entity_data(v):
    return EuVatValidationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return EuVatValidationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return EuVatValidationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return EuVatValidationTestRunner.load_test_control()


def live_client_options():
    return EuVatValidationTestRunner.live_client_options()


def live_delay_ms():
    return EuVatValidationTestRunner.live_delay_ms()
