...
class TestVatDirect:
    ...
    def test_should_direct_load_vat(self):
        setup = _vat_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-vat", "live" if setup["live"] else "unit")
        if _skip:
