...
from euvatvalidation_sdk.utility import register

# Load features
from euvatvalidation_sdk.feature.base_feature import EuVatValidationBaseFeature
from euvatvalidation_sdk.features import _has_feature, _make_feature
...
class EuVatValidationSDK:
    ...
    def get_utility(self):
        return EuVatValidationUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("EuVatValidationSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def ValidateFormat(self, data=None) -> "ValidateFormatEntity":
        """Entity factory: client.ValidateFormat().list() / client.ValidateFormat().load({"id": ...})."""
        from euvatvalidation_sdk.entity.validate_format_entity import ValidateFormatEntity
        return ValidateFormatEntity(self, data)


    def Vat(self, data=None) -> "VatEntity":
        """Entity factory: client.Vat().list() / client.Vat().load({"id": ...})."""
        from euvatvalidation_sdk.entity.vat_entity import VatEntity
        return VatEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "EuVatValidationSDK":
        if sdkopts is None:
            sdkopts = {}
