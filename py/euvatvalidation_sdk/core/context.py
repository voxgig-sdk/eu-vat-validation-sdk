...
from euvatvalidation_sdk.core.helpers import get_ctx_prop, to_map
...
class EuVatValidationContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = EuVatValidationControl()
        ...
        if isinstance(sp, EuVatValidationSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, EuVatValidationResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, EuVatValidationResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return EuVatValidationOperation({})
    ...
    def make_error(self, code, msg):
        return EuVatValidationError(code, msg, self)
