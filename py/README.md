# EuVatValidation Python SDK



The Python SDK for the EuVatValidation API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.ValidateFormat()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/eu-vat-validation-sdk/releases)) or
...
client = EuVatValidationSDK({
    "apikey": os.environ.get("EU_VAT_VALIDATION_APIKEY"),
})
```
...
validateformat = client.ValidateFormat().load({"country": "example", "number": "example"})
# validateformat contains the mock response record
```
...
| `vat_number_full` | `str` | Full VAT number including country code prefix |
...
| --- | --- | --- |
| `checked_at` | `str` | Timestamp of the validation check |
| `company_address` | `str` | Registered company address from VIES |
...
```
py/
├── euvatvalidation_sdk.py         -- Main SDK module
