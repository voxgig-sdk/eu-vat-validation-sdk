# eu-vat-validation-cli

boru-driven command-line client **and** interactive REPL for the EuVatValidation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/eu-vat-validation-cli)
make build

# 2. See usage (words, entities, env vars)
./eu-vat-validation-cli --help

# 3. Provide credentials once, via the environment
export EU_VAT_VALIDATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./eu-vat-validation-cli load 1 validate_format            # {id:1} shorthand
./eu-vat-validation-cli load '{id:1}' validate_format       # explicit match map

# 5. Override the API base URL for a single call
EU_VAT_VALIDATION_BASE=https://api.example.com ./eu-vat-validation-cli load 1 validate_format

# 6. No arguments -> interactive REPL
./eu-vat-validation-cli
eu-vat-validation> load 1 validate_format
eu-vat-validation> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/eu-vat-validation-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/eu-vat-validation-cli load 1 validate_format
   ```
...
```sh
./eu-vat-validation-cli load 1 validate_format          # scalar shorthand for {id:1}
./eu-vat-validation-cli load '{id:1}' validate_format     # explicit match map
```
...
./eu-vat-validation-cli load 1 validate_format
```
...
Run with no arguments to open a REPL (prompt `eu-vat-validation>`). Each line is
evaluated as its own boru expression:

```text
$ ./eu-vat-validation-cli
eu-vat-validation> load 1 validate_format
eu-vat-validation> /help
eu-vat-validation> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `validate_format`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/eu-vat-validation-cli`. |
...
(`./eu-vat-validation-cli <expr>`) and interactively (the REPL), and expressions compose the
