...
// EuVatValidation SDK ValidateFormat entity
...
class ValidateFormatEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ValidateFormatEntity($this->_client, $opts);
    }

    /**
     * @param ValidateFormat|array $args ValidateFormat data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = EuVatValidationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ValidateFormat|array The current ValidateFormat data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ValidateFormat fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = EuVatValidationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ValidateFormat fields).
     */
    ...
    /**
     * Load a single ValidateFormat.
     *
     * @param ValidateFormatLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ValidateFormatLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ValidateFormat|array The loaded ValidateFormat as an assoc-array at the
     *   SDK boundary; throws EuVatValidationError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = EuVatValidationHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
