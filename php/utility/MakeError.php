...
// EuVatValidation SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class EuVatValidationMakeError
{
    public static function call(?EuVatValidationContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new EuVatValidationContext([], null);
        }
        $op = $ctx->op ?? new EuVatValidationOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new EuVatValidationResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof EuVatValidationError) ? $err->msg : (string)$err;
        $msg = "EuVatValidationSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new EuVatValidationError('', $msg, $ctx);
        ...
        if ($err instanceof EuVatValidationError) {
