...
class VatEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = EuVatValidationSDK::test(null, null);
        $ent = $testsdk->Vat(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = vat_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "vat." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $vat_ref01_data = null;
        if (count($vat_ref01_data_raw) > 0) {
            $vat_ref01_data = Helpers::to_map($vat_ref01_data_raw[0][1]);
        }

        // LOAD
        $vat_ref01_ent = $client->Vat(null);
        $vat_ref01_match_dt0 = [
            "id" => $vat_ref01_data["id"],
        ];
        $vat_ref01_data_dt0_loaded = $vat_ref01_ent->load($vat_ref01_match_dt0, null);
        $vat_ref01_data_dt0_load_result = Helpers::to_map(is_object($vat_ref01_data_dt0_loaded) && method_exists($vat_ref01_data_dt0_loaded, 'data_get') ? $vat_ref01_data_dt0_loaded->data_get() : $vat_ref01_data_dt0_loaded);
        $this->assertNotNull($vat_ref01_data_dt0_load_result);
        $this->assertEquals($vat_ref01_data_dt0_load_result["id"], $vat_ref01_data["id"]);

    }
}

function vat_basic_setup($extra)
{
    Runner::load_env_local();

    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/vat/VatTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["vat01", "vat02", "vat03", "country01"] as $k) {
