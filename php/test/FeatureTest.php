...
// EuVatValidation SDK feature test
//
...
class FtCtrl extends EuVatValidationControl
{
...
class FtClient
{
    ...
    private EuVatValidationUtility $utility;

    public function __construct(EuVatValidationUtility $utility, array $options)
    {
    ...
    public function get_utility(): EuVatValidationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EuVatValidationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EuVatValidationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (EuVatValidationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EuVatValidationConfig::make_feature($name);
    ...
    private static function buildUrl(EuVatValidationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EuVatValidationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EuVatValidationResponse($fetched);
        }
    ...
    private function fail(EuVatValidationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EuVatValidationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
