...
// EuVatValidation SDK pipeline test
//
...
{
    private static function utility(): EuVatValidationUtility
    {
        return new EuVatValidationUtility();
    }
    ...
    private static function ctx(array $over = []): EuVatValidationContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new EuVatValidationContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new EuVatValidationOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof EuVatValidationError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new EuVatValidationOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = EuVatValidationMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new EuVatValidationOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = EuVatValidationMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new EuVatValidationOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = EuVatValidationMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EuVatValidationSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = EuVatValidationMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new EuVatValidationResponse([]),
            'result' => new EuVatValidationResult([]),
        ]);
        [, $err] = EuVatValidationMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new EuVatValidationSpec([]),
            'response' => null,
            'result' => new EuVatValidationResult([]),
        ]);
        [, $err] = EuVatValidationMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new EuVatValidationSpec(['step' => 's']),
            'response' => new EuVatValidationResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new EuVatValidationResult([]),
        ]);
        [, $err] = EuVatValidationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new EuVatValidationSpec(['step' => 's']),
            'response' => new EuVatValidationResponse(self::resp(200, ['v' => 1])),
            'result' => new EuVatValidationResult([]),
        ]);
        [, $err] = EuVatValidationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new EuVatValidationSpec(['step' => 's']),
            'response' => new EuVatValidationResponse(self::resp(200, ['v' => 2])),
            'result' => new EuVatValidationResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        EuVatValidationMakeResponse::call($ctx);
    ...
    {
        $preset = new EuVatValidationResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new EuVatValidationSpec([]),
            'response' => new EuVatValidationResponse([]),
            'result' => new EuVatValidationResult([]),
        ...
        [$got, $err] = EuVatValidationMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new EuVatValidationResult([])]);
        [, $err] = EuVatValidationMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new EuVatValidationSpec([]), 'result' => null]);
        [, $err] = EuVatValidationMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EuVatValidationSpec(['step' => 's']),
            'result' => new EuVatValidationResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new EuVatValidationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EuVatValidationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EuVatValidationSpec(['step' => 's']),
            'result' => new EuVatValidationResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new EuVatValidationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EuVatValidationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EuVatValidationResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new EuVatValidationSpec([]),
            'result' => new EuVatValidationResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = EuVatValidationMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EuVatValidationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new EuVatValidationError('boom', 'boom');
        $utility->fetcher = function (EuVatValidationContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EuVatValidationSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = EuVatValidationMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (EuVatValidationContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = EuVatValidationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(EuVatValidationResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        EuVatValidationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (EuVatValidationContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EuVatValidationSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = EuVatValidationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new EuVatValidationResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new EuVatValidationSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = EuVatValidationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = EuVatValidationMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = EuVatValidationMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new EuVatValidationResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, EuVatValidationDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new EuVatValidationResult(['ok' => false])]);
        $this->expectException(EuVatValidationError::class);
        EuVatValidationDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new EuVatValidationResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', EuVatValidationMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        EuVatValidationMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new EuVatValidationBaseFeature();
        $b = new EuVatValidationBaseFeature();
        EuVatValidationFeatureAdd::call($ctx, $a);
        EuVatValidationFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): EuVatValidationBaseFeature
    {
        $f = new EuVatValidationBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        EuVatValidationFeatureAdd::call($ctx, self::named_feature('a'));
        EuVatValidationFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        EuVatValidationFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        EuVatValidationFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        EuVatValidationFeatureAdd::call($ctx, $replace);
        ...
        EuVatValidationFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): EuVatValidationContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new EuVatValidationSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EuVatValidationPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        EuVatValidationPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new EuVatValidationResponse(['headers' => null]),
            'result' => new EuVatValidationResult([]),
        ]);
        EuVatValidationResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        EuVatValidationResultBody::call($ctx);
