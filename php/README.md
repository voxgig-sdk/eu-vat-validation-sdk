# EuVatValidation PHP SDK



The PHP SDK for the EuVatValidation API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->ValidateFormat()` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/eu-vat-validation-sdk/releases](https://github.com/voxgig-sdk/eu-vat-validation-sdk/releases)
...
```php
<?php
require_once 'euvatvalidation_sdk.php';

$client = new EuVatValidationSDK([
    "apikey" => getenv("EU_VAT_VALIDATION_APIKEY"),
]);
```
...
try {
    ...
    print_r($validateformat->data_get());
} catch (\Throwable $err) {
...
```php
require_once 'euvatvalidation_sdk.php';
$client = new EuVatValidationSDK($options);
```
...
```php
$client = EuVatValidationSDK::test($testopts, $sdkopts);
```
...
Create an instance: `$validate_format = $client->ValidateFormat();`
...
Create an instance: `$vat = $client->Vat();`
...
```
php/
├── euvatvalidation_sdk.php          -- Main SDK class
...
The main class (`euvatvalidation_sdk.php`) exports the SDK class
