...
EU VAT Validation API client, generated from the OpenAPI spec.

This API enables real-time validation of EU VAT numbers using the official VIES service.

Learn more about EU VAT Validation API at [kiprio.com](https://kiprio.com).

This is an unofficial SDK for the EU VAT Validation public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — ValidateFormat and Vat — that you
...
```go
client := sdk.Test()
result, err := client.ValidateFormat(nil).Load(
    nil, nil,
)
```
...
// Load a specific validateformat (returns a ValidateFormat)
...
```bash
cd go-mcp && go build -o eu-vat-validation-mcp .
```
...
```json
{
  "mcpServers": {
    "eu-vat-validation": {
      "command": "/abs/path/to/eu-vat-validation-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **ValidateFormat** | The ValidateFormat entity (load). | `/vat/validate-format/{country}/{number}` |
| **Vat** | The Vat entity (load). | `/vat/{country}/{number}` |
...
# Load a specific validateformat (returns the record, raises on error)
...
```go
import sdk "github.com/voxgig-sdk/eu-vat-validation-sdk/go"
...
// Load a specific validateformat
validateFormat, err := client.ValidateFormat(nil).Load(
    map[string]any{"country": "example_country", "number": "example_number"}, nil,
)
if err != nil {
    panic(err)
}
...
Generated from the EU VAT Validation API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
