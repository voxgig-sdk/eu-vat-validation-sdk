...
const __1 = require("..");
const SDK_NAME = 'EuVatValidationSDK';
...
const TEST_SEED = { "entity": { "validate_format": { "test01": { "id": "test01" } }, "vat": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+EuVatValidationSDK\b/.test(code) ||
        /\bEuVatValidationSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+EuVatValidationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/EuVatValidationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
