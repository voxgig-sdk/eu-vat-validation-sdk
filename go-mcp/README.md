# eu-vat-validation-mcp

[MCP](https://modelcontextprotocol.io) server exposing the EuVatValidation SDK as
two agent tools — `eu-vat-validation_list` and `eu-vat-validation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/eu-vat-validation-mcp)
make build

# 2. Provide credentials via the environment
export EU_VAT_VALIDATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user eu-vat-validation \
  -- /absolute/path/to/eu-vat-validation-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./eu-vat-validation-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// eu-vat-validation_list: first page of records
{ "entity": "validate_format" }
{ "entity": "validate_format", "query": { } }

// eu-vat-validation_load: one record by id
{ "entity": "validate_format", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/eu-vat-validation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user eu-vat-validation \
     -- "$PWD"/dist/*/eu-vat-validation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `eu-vat-validation_list` and `eu-vat-validation_load` tools now appear
   in new sessions. Ask the agent to *"list validate_format using eu-vat-validation"*
   and it calls `eu-vat-validation_list` with `{"entity":"validate_format"}`.
...
```sh
export EU_VAT_VALIDATION_APIKEY=sk_live_xxx            # API key
export EU_VAT_VALIDATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `eu-vat-validation_list` tool
...
### Call the `eu-vat-validation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `eu-vat-validation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `eu-vat-validation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `EU_VAT_VALIDATION_APIKEY` | API key sent with every request. |
| `EU_VAT_VALIDATION_BASE` | Optional override of the API base URL. |
...
```sh
./eu-vat-validation-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"eu-vat-validation_load","arguments":{"entity":"validate_format","query":{"id":1}}}}'
```
