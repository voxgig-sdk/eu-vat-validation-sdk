...
module EuVatValidationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = EuVatValidationResponse.new({})
    ...
    if fetch_err
      ...
      response = EuVatValidationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EuVatValidationResponse.new(fetched)
    else
