...
class ValidateFormatEntity
  ...
  def make
    opts = @_entopts.dup
    ValidateFormatEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = EuVatValidationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ValidateFormat, Hash] the current ValidateFormat data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = EuVatValidationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ValidateFormat fields)
  def match_get
  ...
  #
  # @param reqmatch [ValidateFormatLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ValidateFormat.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ValidateFormat, Hash] the loaded ValidateFormat; raises EuVatValidationError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = EuVatValidationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
